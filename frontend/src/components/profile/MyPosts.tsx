...
const MyPosts = () => {
    ...
    if (!isLoading && !error && content.length === 0) {
        showContent = <Message message="Non hai nessun Articolo..," />;
    }
    if (!isLoading && !error && content.length > 0) {
        showContent = (
            <div>
    ...
    const handleDeletePost = async () => {
        try {
            ...
            if (res?.message) {
                ...
                toast.success("Il Post è stato eliminato!");
            }
    ...
    const handleUpdatePost = async () => {
        ...
        if (Object.keys(getValidData).length === 0) {
            try {
                ...
                toast.success("Il Post è stato aggiornato");
    ...
    if (!isCatLoading && categories.length === 0 && !catError)
        showCategories = <Message message={"Categories non avviabile"} />;

    const buttonDisabled =
        isCatLoading || isCatLoading || !title || !editorContent;
    return (
        <div className="py-10">
            <h1 className="text-2xl font-semibold text-gray-600">I tuoi Articoli: </h1>
            {showContent}
